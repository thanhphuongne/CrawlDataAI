...
export class SelectService {
  ...
  */
  ...
  public getInternalSelection(): SelectableItem[] {
    ...
    if (selectedItems && selectedItems.length > 0) {
      ...
      const max = this._options.maxVisibleItemCount
        ? this._options.maxVisibleItemCount
        : 0;
      ...
      for (const item of selectedItems) {
        item.isVisible =
          ...
          (!item.hasChild ||
            item.children.every(child => child.selected === false));
  ...
  private setExpandForList(items: SelectableItem[], isCollapse?: boolean) {
    if (!items) {
      return;
    }
    for (const item of items) {
      this.setExpandForList(item.children);
      item.isOpen = isCollapse
        ? false
        : this._options.filterExpandMode === ExpandMode.All;
  ...
  public setCollapse() {
    this.setExpandForList(this._items, true);
  }

  private toSelectableItems(
    sources: any[],
    parent?: SelectableItem
  ): SelectableItem[] {
    if (sources && Array.isArray(sources)) {
      ...
      return sources.map(srcItem => {
        ...
        const fullText = srcItem[this._options.fulltextProperty];
        item.fullText = fullText ? fullText : item.text;
        if (this._options.isHierarchy()) {
          if (parent) {
            item.parent = parent;
            item.fullText = parent.fullText + '.' + item.text;
          }
          item.children = this.toSelectableItems(
            srcItem[this._options.childProperty],
            item
          );
        }
        return item;
      ...
      for (const v of array) {
        if (v.hasChild && v.selected === true) {
          if (v.children.every(child => child.selected === false)) {
            ...
            // res = [...res, ...this.getSelectedItems(v.children)]; // old implement
            res = [...res, v, ...this.getSelectedItems(v.children)];
          }
        } else if (v.hasChild) {
  ...
  private checkConfig() {
    if (
      this.Configuration.allowMultiple &&
      this.Configuration.closeOnSelection
    ) {
      ...
      if (value) {
        if (typeof value !== 'object') {
          ...
          if (value[this.Configuration.idProperty]) {
            if (
              v.id === (value[this.Configuration.idProperty] || '').toString()
            ) {
              result.push(v);
            }
          }
          if (
            this.Configuration.isHierarchy() &&
            v.children &&
            v.children.length > 0
          ) {
